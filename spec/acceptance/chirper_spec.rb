describe "Creating dem chirps" do
  ...
  it "allows me to post and view chirps" do
    ...
    create_new_chirps.execute(username: 'Test One', chirp: 'I just want to pet all the cats.')
    create_new_chirps.execute(username: 'Test Two', chirp: 'All dogs are good dogs.')

    chirps = view_chirps.execute
    expect(chirps[0]).to eq({username: 'Test One', chirp: 'I just want to pet all the cats.'})
    expect(chirps[1]).to eq({username: 'Test Two', chirp: 'All dogs are good dogs.'})
  end
end
